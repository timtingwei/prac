...
#lines = rs.ExplodeCurves(rec)

t = door_location
t_len = door_length
#t_len = (door_length*4)/(rs.CurveLength(rec))
t_thick = (4*wall_thickness)/(rs.CurveLength(rec)) #墙厚在curve_rec上的t
print (rs.CurveDomain(rec))
#ERROR:在wall的墙角处对数据进行修正，主要改变new_pt1的值，new_pt2由new_pt1得到
#t_thickness,pt_dist怎么求
#new_pt2 = new_pt1 + pt_dist

#door的控制方法：尽量减少输入的参数，在函数内部对数据进行控制
#(a) length一定，到边界处时候，门就要转到另外一边
#(b) door占据边的比例，占剩余部分的比例
    #(b1) door占剩余边的比例(length mapping...)
new_pt1 = rs.CurveDomain(rec)[1]*t
print ('new_pt1:',new_pt1)
#创建门的第二点，point在curve的不同区间要有变化，point不能超出rec
if t_len<=0:
    new_pt2 = (new_pt1-int(new_pt1)-t_thick)*t_len 
else :
    new_pt2 = (int(new_pt1)+1-new_pt1-t_thick)*t_len
print ('new_pt2:',new_pt2)
print ('new_pt1-int(new_pt1):',new_pt1-int(new_pt1))
print ('new_pt1-int(new_pt1)-t_thick :',new_pt1-int(new_pt1)-t_thick)
door_pt1 = rs.EvaluateCurve(rec,new_pt1)
door_pt2 = rs.EvaluateCurve(rec,new_pt1 + new_pt2)
...
#wall_polyline2 = rs.GetObject(wall_polyline2)
